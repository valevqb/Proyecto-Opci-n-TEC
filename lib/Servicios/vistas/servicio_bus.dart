...
class _InformacionBusState extends State<InformacionBus> {
            ...
            appBar: AppBar(
              ...
              title: Text('Buses',
                  style: TextStyle(
                      ...
                      color: Colors.white)),
              elevation: 0,
              backgroundColor: Color(0xFF2B436D),
            ),
            body: (isLoading)
  ...
  Widget seleccionOrigen(BuildContext context, List<dynamic>? servicioBus) {
    //titulo de la pagina e imagen
    return Container(
        //La imagen donde dice servicios
        height: 286,
        width: (MediaQuery.of(context).size.width),
        padding: const EdgeInsets.only(top: 7.8, left: 24.5),
        decoration: const BoxDecoration(
            color: Color(0xFF2B436D),
            image: DecorationImage(
              image: NetworkImage(''), //Aca va la imagen
  ...
  Widget nombreDestino(BuildContext context, List<dynamic>? servicioBus) {
    List<dynamic> listaOrigenes = ["Origen"];
    List<List<dynamic>> listaDestino = [
      ["Destino"]
    ];
    ...
    return Container(
      ...
      margin:
          const EdgeInsets.only(top: 34.7, bottom: 59.5, left: 24, right: 24),
      child: Align(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            Container(
              margin: const EdgeInsets.only(left: 13, right: 74),
              child: Column(
                children: [
                  Container(
                    height: 56,
                    child: dropInformacionOrigen(
                        context, listaOrigenes, servicioBus),
                  ),
                  Container(
                    height: 56,
                    child: dropInformacionDestino(context, listaDestino),
                  )
                ],
              ),
            )
  ...
  Widget dropInformacionOrigen(BuildContext context, List<dynamic>? informacion,
      List<dynamic>? servicioBus) {
    //Informacion de cada dropdown
    return DropdownButton(
      isExpanded: true,
      value: _origen,
      items: informacion
          ?.map(
            (lists) => DropdownMenuItem(
                value: lists.toString(),
                child: Padding(
                  padding: EdgeInsets.only(left: 8),
                  child: Text(lists.toString(),
                      style: Theme.of(context).textTheme.bodyLarge),
                )),
          )
          ?.toList(),
      onChanged: (String? newValue) {
        setState(() {
          ...
          _origen = newValue.toString();
          _index = informacion!
              .indexWhere((element) => element.toString() == _origen);
        });
      },
    );
  }

  Widget dropInformacionDestino(
      BuildContext context, List<List<dynamic>> informaciones) {
    //Informacion de cada dropdown
    List<dynamic> informacion = informaciones[_index];
    ...
    return DropdownButton(
      ...
      onChanged: (String? newValue) {
        setState(() {
          ...
          _index2 = informacion!
              .indexWhere((element) => element.toString() == _destino);
        });
      },
    );
  }

  Widget informacionServicio(BuildContext context, List<dynamic>? servicioBus) {
    ...
    return Column(
        ...
        children: [
          Container(
            padding: const EdgeInsets.only(top: 23.39, left: 24, right: 24),
            child: Text(
              "Aquí podés encontrar todas las rutas de buses para movilizarte al TEC de Cartago. ",
              //Labor del servicio
              style: Theme.of(context).textTheme.bodySmall,
            ),
          ),
          Row(
            ...
            children: [
              Container(
                  padding: const EdgeInsets.only(top: 20.60, left: 24),
                  alignment: Alignment.topLeft,
                  child: dataBusServicio(
                      context,
                      "Precio",
  ...
  Widget dataBusServicio(BuildContext context, titulo, cantidad, imagen) {
    return Row(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Image(width: 24, height: 24, image: AssetImage(imagen)),
        Container(
          margin: const EdgeInsets.only(left: 5.5),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.start,
